...
def main():
    ...
    Ahat, Bhat, Chat = model_reduction.get_projected_system(A, B, C, V, V)
    ...
    plt.loglog(w, mag2, 'r--', label='k = {:d} (Galerkin)'.format(k))
    plt.legend()
    ...
    plt.subplot(1, 2, 2)
    plt.loglog(w, mag, label='n = {:d} (full-model)'.format(A.shape[0]))
    plt.loglog(w, mag3, 'r--', label='k = {:d} (Petrov-Galerkin)'.format(k))
    plt.legend()
    plt.title('Bode Diagram')
    plt.xlabel('Frequency (rad/sec)')
    plt.ylabel('Magnitude (dB)')
